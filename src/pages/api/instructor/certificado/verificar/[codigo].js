// api/instructor/certificado/verificar/[codigo].js
...
async function handler(req, res) {
  ...
  try {
    ...
    const result = await query(`
      ...
      WHERE cert.codigo_verificacao = $1 AND c.instrutor_id = $2
    `, [codigo, user.id]);

    if (result.rows.length === 0) {
      return res.status(404).json({ 
        success: false,
        message: 'Certificado não encontrado ou você não é o instrutor deste curso',
        valid: false
      });
    }
